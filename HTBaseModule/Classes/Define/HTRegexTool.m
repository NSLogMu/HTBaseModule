...
+ (BOOL) validateIdentityCard: (NSString *)value
{
    ...
    //
    //    //校验生日
    //    NSUInteger size = identityCard.length ;
    //    NSInteger birthYear = 0;    //生日年份
    //    if(size == 15){
    //        birthYear = [[identityCard substringWithRange:NSMakeRange(6, 2)] integerValue]+1900;
    //        if(birthYear % 400 == 0 ||
    ...
    //            NSPredicate *identityCardPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex1];
    //            return [identityCardPredicate evaluateWithObject:identityCard];
    //        }else{
    ...
    //    else if (size == 18)
    //    {
    //        birthYear = [[identityCard substringWithRange:NSMakeRange(6, 4)] integerValue];
    //        if(birthYear % 400 == 0 ||
    //           (birthYear % 100 != 0 && birthYear % 4 == 0))
    //        {
    //            NSString *regex1 = @"^[1-9][0-9]{5}(19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|3[0-1])|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|[1-2][0-9]))[0-9]{3}[0-9Xx]$";// 闰年出生日期的合法性正则表达式
    //            NSPredicate *identityCardPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex1];
    //            return [identityCardPredicate evaluateWithObject:identityCard];
    //        }else{
    //            NSString *regex1 = @"^[1-9][0-9]{5}(19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|3[0-1])|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|1[0-9]|2[0-8]))[0-9]{3}[0-9Xx]$";// 平年出生日期的合法性正则表达式
    //            NSPredicate *identityCardPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex1];
    //            return [identityCardPredicate evaluateWithObject:identityCard];
    //        }
    //    }
    ...
    switch (length) {
        ...
        case 18:
            ...
            if(numberofMatch >0) {
                ...
                //添加校验位为x的情况
                NSString *JYXM = @"10x987654321";
                M1 = [JYM substringWithRange:NSMakeRange(Y,1)];// 判断校验位
                M2 = [JYXM substringWithRange:NSMakeRange(Y,1)];// 判断校验位
                if ([M1 isEqualToString:[value substringWithRange:NSMakeRange(17,1)]] || [M2 isEqualToString:[value substringWithRange:NSMakeRange(17,1)]]) {
                    return YES;// 检测ID的校验位
                }else {
                    return NO;
                }
...
+ (BOOL) validateSoliderID: (NSString *)text{
    NSString *soliderIDRegex = @"^[a-zA-Z0-9]{0,}$";
